...
const Blanks: React.FC<QidProps> = ({ document }) => {
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (document) {
      setLoading(false);
      if (!document.overallFeedback_for_student || document.overallFeedback_for_student.trim() === "") {
        toast({
          title: "No Feedback Yet",
          description: "Our AI system will grade your assignment soon. Please check back in a few minutes.",
        });
      }
    }
  }, [document]);
      ...
      <Card className="mb-6  rounded-lg border border-gray-200">
        <CardHeader className="pb-0">
          ...
          <p className="text-2sm">
            Submission Date: {document && format(new Date(document.submissionDate), "MMMM dd, yyyy")}
          </p>
          <p className="text-2sm">
        ...
        <CardContent>
          ...
          {document?.selectedanswer.map((qa, index) => (
            <div key={index} className="mb-6">
              ...
              <p className="text-base mb-2 text-gray-600">Score: {qa.score}</p>
              <div className="mt-2">
                <ReactMarkdown className="markdown-content">
                  {qa.feedback || "Feedback not available"}
                </ReactMarkdown>
              </div>
              <Separator className="my-4" />
            </div>
          ))}
          <h3 className="text-lg font-semibold mb-4">Overall Feedback for Student</h3>
          <div className="mb-4">
            {document ? (
              <ReactMarkdown className="markdown-content">
                {document.overallFeedback_for_student || "No feedback available yet"}
              </ReactMarkdown>
            ) : (
